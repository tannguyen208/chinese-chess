...
const elChkRotate = document.getElementById('chkRotate');
...
const MODALS = ['mod_gameselect', 'mod_shareurl', 'mod_waiting', 'mod_disconnected', 'mod_computer', 'mod_gameover'];
...
let main = function () {
  ...
  let start = function (computer = false, skill = 0) {
    ...
    let onmove = function (evt) {
      ...
      if (storage.getItem('notiSound') === 'enabled') {
        if (move.check) {
          notify.playSound(elSndCheck);
        } else if (move.capture) {
          notify.playSound(elSndCapture);
        } else {
          notify.playSound(elSndMove);
        }
      }
    };
  ...
  elChkRotate.addEventListener('change', function (evt) {
    const status = elChkRotate.checked ? 'enabled' : 'disabled';
    elBoard.setAttribute('data-rotate', status);
  });
