...
export default class Board extends EventTarget {
  constructor(element, computer, online, fen) {
    ...
    for (let j = 0; j < ROWS; ++j) {
      ...
      row.setAttribute('data-pos', ROWS - j);
      row.setAttribute('data-latest', ROWS - 1 === j);

      for (let i = 0; i < COLS; ++i) {
        let cell = document.createElement('td');
        cell.setAttribute('data-pos', COLS - i + 1);
    ...
    for (let sq = 0; sq < 256; ++sq) {
      ...
      img.setAttribute('data-index', sq);
  ...
  postAddMove(mv, computerMove) {
    ...
    if (this.pos.isMate()) {
      ...
      if (!sqMate) {
        ...
        this.imgSquares[sqMate].src = RESOURCES + (this.pos.sdPlayer == 0 ? 'r' : 'b') + 'km.svg';
        this.postMate(computerMove);
        return;
      }
      sqMate = this.flipped(sqMate);
